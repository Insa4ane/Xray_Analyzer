...
@patch.object(XrayAgent, "save_history_and_results", return_value=True)
@patch.object(XGBoostAgent, "save_results", return_value=True)
def test_train_success(mock_results,mock_history_and_results, mock_data, mock_count, mock_run_agent, mock_run_xgb, mock_save_agent, mock_save_xgb):
...
@patch.object(DataLoader, "split_data")
def test_train_exception(mock_data):
    mock_data.side_effect=Exception("Error! we cannot split data")
    result=train.main()
    assert not result
